...
class RegistrationFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        registrationButton?.setOnClickListener {
            val user = LoginRequest("silf","123")
            //val user = RegistrationData(registrationUser.text.toString(), registrationPassword.text.toString())
    ...
    private fun postRegistration(registrationData: LoginRequest) {

        val clientR = RetrofitService.create()

        disposable = clientR.postRequest(registrationData)
            .observeOn(AndroidSchedulers.mainThread())
            .subscribeOn(Schedulers.io())

            .subscribe(
                { response -> onResponse(response) },
                //{ result -> Log.v("POSTED ARTICLE", "" + registrationUser) },
                {  t -> onFailure(t) }
            )
    }
